...
{
	public class PreviousRankedPage : ContentPage
	{
		ListView listView;
		static ObservableCollection<GroupedPrevious> grouped { get; set; }

		public PreviousRankedPage(ObservableCollection<GroupedPrevious> grouped)
		{
			Title = "Previous Game";
		...
		public static async Task<PreviousGamePage> loadPrevious(MatchDetail thisGame, Region region, Summoner summonerPls,int champId, List<RiotSharp.MatchEndpoint.Participant> fellowPlayers, List<RiotSharp.MatchEndpoint.ParticipantIdentity> fellowPt2)
		{
			...
			//playersInPrev.Add(new playersInPrevious { ChampionId = champId, SummonerId = (int)summonerPls.Id });
			for (int i = 0; i < fellowPlayers.Count; i++)
			{
				playersInPrev.Add(new playersInPrevious { ChampionId = fellowPlayers[i].ChampionId, SummonerId = (int)fellowPt2[i].Player.SummonerId });
			}
